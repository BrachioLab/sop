...
for method in "${methods[@]}"; do
    python scripts/python/text/multirc/eval_multirc_baselines_save.py "$method"
done

# eval all saved attributions
bash scripts/bash/eval_multirc_from_save.sh

# generate tables
python scripts/python/gen_results_table.py multirc_5e-06 multirc