...
def get_explainer(original_model, original_model_softmax, backbone_model, processor, 
...
def get_attr_from_explainer(explainer, explainer_name, inputs, preds, processor, kwargs={}, device='cuda',
    ...
    elif explainer_name in ['rise', 'attn']:
...
def get_expln_all_classes(original_model, inputs, explainer, num_classes, explainer_name, processor, kwargs={}):
    # bsz, num_channels, H, W = inputs.shape
    bsz, seq_len = inputs.shape

    logits = original_model(inputs, **kwargs)
    ...
    expln = get_attr_from_explainer(explainer, explainer_name, inputs, multi_labels.clone(), processor, 
        kwargs=kwargs, device=inputs.device, return_expln=True)
