...
def get_dataset(dataset_name, split='val', num_data=-1, start_data=0, batch_size=16, shuffle=False,
    ...
    def transform(batch):
        ...
        if processor is not None:
            batch['passage'] = batch['passage'].replace('\n', ' ')
            inputs = processor(batch['passage'], 
                            batch['query'], 
                            padding='max_length', 
                            truncation=True, 
            ...
            # import pdb; pdb.set_trace()
            # batch['evidences'] = [[evidence['text'] for evidence in example] for example in batch['evidences']]
            batch['evidences'] = [evidence['text'] for evidence in batch['evidences'][0]]
            batch['passage'] = batch['passage'].replace('\n', ' ')
            return batch

    
    if dataset_name == 'movies':
        DATA_DIR = '/shared_data0/weiqiuy/sop/data/movies/movies'
        if split == 'train':
            dataset = EraserMovies(DATA_DIR, split='train',
                                         transform=transform, data_size=num_data)
        elif split == 'val':
            dataset = EraserMovies(DATA_DIR, split='dev',
                                         transform=transform, data_size=num_data)
        elif split == 'train_val':
            dataset = EraserMovies(DATA_DIR, split='train',
                                         transform=transform, data_size=num_data)
        elif split == 'test':
            dataset = EraserMovies(DATA_DIR, split='test',
                                         transform=transform, data_size=num_data)
        else:
