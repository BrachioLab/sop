...
def get_masks_from_mask(attribution, explainer_name, k=0.2, deletion=False):
...
def get_ins_del_perc_cosmo(explainer_name, backbone_model, original_model, model, dark_thresh=0, bright_thresh=3, ks=np.linspace(0.1, 1, 10),
    ...
    # for i in range(len(filenames)):
        ...
        if explainer_name == 'sop':
            ...
            for k in ks:
                ...
                model.k = k
                outputs = model(image[None], separate_scale=True, deletion=deletion)
                logits = outputs
        ...
        loss_original.append(criterion(logits, label))
    ...
    loss_original = torch.stack(loss_original)

    print('masked loss', torch.mean(loss_all), 'original loss', torch.mean(loss_original))
